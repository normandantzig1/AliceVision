                                           ...
                                           bool nearestFarest, Point3d& outIntersectPt) const
{
    ...
    {
        // Do not test the intersection with the previous facet if known.
        if(inFacet.cellIndex != GEO::NO_VERTEX && inFacet.cellIndex == i)
            continue;
...
DelaunayGraphCut::Facet DelaunayGraphCut::getFirstFacetOnTheRayFromCamToThePoint(int cam, const Point3d& p, Point3d& intersectPoint) const
{
    ...
    for(CellIndex adjCellIndex : getNeighboringCellsByVertexIndex(cam_vi)) // GEOGRAM: set_stores_cicl(true) required
    {
        ...
        Facet outFacet;
        // Unknown precise input facet (define only cellIndex with adjCellIndex)
        Facet inFacet(adjCellIndex, GEO::NO_VERTEX);
        if(rayCellIntersection(camBehind, mp->CArr[cam], inFacet, outFacet, false, intersectPoint))
        {
...
#pragma omp parallel for
    for(int i = 0; i < vetexesToProcessIdsRand->size(); ++i)
    {
        ...
        for(int c = 0; c < v.cams.size(); ++c)
        {
            ...
            const bool nearestFarest = true;
            const Facet fFirst = getFacetFromVertexOnTheRayToTheCam(vi, cam, nearestFarest);
            ...
            {
                const float eFirst = _cellsAttr[fFirst.cellIndex].out;
        ...
        for(int c = 0; c < v.cams.size(); ++c)
        {
...

            {
                ...
                while(ok)
                {
                    ...
                    {
                        // Take the mirror facet to iterate over the next cell
                        facet.cellIndex = mirrorFacet(outFacet).cellIndex;
                        if(facet.cellIndex == GEO::NO_CELL)
                            ok = false;
                        p = intersectPt;
                    }
